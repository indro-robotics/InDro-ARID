...
# Single-network model: a join leaves every other network first, so joining over an existing
# ZeroTier session drops that session.
set -u
...
report() {
    ...
    echo "======================================================"
    # Without a terminal this must not block: setup.sh runs it unattended.
    if [[ ! -r /dev/tty ]]; then
