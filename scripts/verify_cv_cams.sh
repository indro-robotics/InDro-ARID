#!/bin/bash
# Live visual check of the downward CV camera (IMX219 CSI), through the gst_camera_manager:
#   pipeline cam_down -> /cam_down/image_raw/compressed
#
# Starts the manager pipeline (if not already running) and streams the published COMPRESSED
# image topic in a cv2 window at the pipeline's native rate. The manager is the single camera
# consumer, so there is no contention. Click the window or press any key in THIS TERMINAL to
# advance. The pipeline is left stopped on exit if this script started it.
#
# Run any time:  ver_cv_cams   or   bash scripts/verify_cv_cams.sh
...
if ! display_ready; then
    ...
    echo "  Connect a NoMachine session to the drone - verification starts automatically once"
    echo "  you are connected. (press Enter to skip)"
    while ! display_ready; do
        if read -r -t 2 _ 2>/dev/null; then warn "camera verification skipped"; exit 0; fi
    done
fi
...
run_feed() {                       # $1 = label, $2 = pipe, $3 = compressed topic
    ...
    if ! ros2 service list 2>/dev/null | grep -q "/gst_camera_manager/${pipe}\$"; then
        warn "manager service for ${pipe} not found - is gst_camera_manager.service running?"
        return 1
    fi
    ...
    echo "  Opening the feed on the NoMachine display - please wait a moment for the window to appear..."
...
case "$rc" in
    ...
    10) exit 0 ;;
    *)  warn "cam_down: no frames - IMX219 absent / nvargus-daemon issue, or another consumer holds sensor-id=0" ;;
esac
