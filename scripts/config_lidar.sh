...
extract_from_arp() {
    ...
    local pat="$1"
    local line
    line=$(grep -E "$pat" "${CAP}" | grep -i "who-has.*tell" | head -1)
    [[ -z "$line" ]] && return 1
    LIDAR_MAC=$(echo "$line" | grep -oE '([0-9a-f]{2}:){5}[0-9a-f]{2}' | head -1)
    HOST_IP=$(echo "$line" | grep -oE 'who-has [0-9.]+' | awk '{print $2}')
    LIDAR_IP=$(echo "$line" | grep -oE 'tell [0-9.]+'    | awk '{print $2}')
    return 0
}
extract_from_udp() {
    # Sets LIDAR_MAC / LIDAR_IP / HOST_IP. arg: grep pattern or "." for all.
    local pat="$1"
    local line tuple
    line=$(grep -E "$pat" "${CAP}" | grep -E 'UDP, length' | head -1)
    [[ -z "$line" ]] && return 1
    LIDAR_MAC=$(echo "$line" | grep -oE '([0-9a-f]{2}:){5}[0-9a-f]{2}' | head -1)
    tuple=$(echo "$line" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+ > [0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | head -1)
    LIDAR_IP=$(echo "$tuple" | awk '{print $1}' | sed -E 's/\.[0-9]+$//')
    HOST_IP=$(echo "$tuple" | awk '{print $3}' | sed -E 's/\.[0-9]+$//')
    return 0
}
