...
# Flags for resources started by this test (cleaned up in Section 7 + EXIT trap).
...
_cleanup_on_exit() {
    ...
    if [[ -n "${CAM_FRONT_STARTED_BY_US}" ]]; then
...
what     "Two services are required: arid_description, gst_camera_manager."
why      "arid_description publishes /robot_description + TF static. The camera manager exposes SetBool/Trigger services that everything else depends on."
SVC_OUT=$(systemctl is-active arid_description.service gst_camera_manager.service 2>&1)
raw "$(paste <(echo -e 'arid_description\ngst_camera_manager') <(echo "${SVC_OUT}"))"

for svc in arid_description.service gst_camera_manager.service; do
...
if [[ -n "${PORT_OUT}" ]]; then
    ...
    # setsid puts the launch in its own process group so Section 7 can killpg the whole tree.
...
why      "Pipeline is configured for ~15 fps (delivered ~16 Hz). Accept >= 6 Hz (~40% of delivered) as pass. Below that there's something wrong upstream (Argus dropping or ISP backpressure)."
...
raw "${STATUS}"
if echo "${STATUS}" | grep -q 'cam_front STOPPED'; then
    pass "cam_front is STOPPED (clean baseline)"
else
    fail "could not establish baseline STOPPED state"
fi
...
why      "If publishers aren't created, the gst_cam_node binary crashed (most common: empty encoding string, bad calibration path)."
TOPICS=$(ros2 topic list 2>/dev/null)
note     "cam_front topics in the graph:"
echo "${TOPICS}" | grep '^/cam_front' | sed 's/^/         /'
for t in /cam_front/image_raw /cam_front/image_raw/compressed /cam_front/camera_info; do
    if echo "${TOPICS}" | grep -q "^$t$"; then
        pass "topic $t published"
    else
        fail "topic $t missing"
...
what     "Subscribe with matching BEST_EFFORT QoS and echo a single header."
why      "This is the TF frame that downstream consumers transform from. Wrong frame_id silently breaks every camera→base_link TF lookup."
HDR_OUT=$(timeout 5 ros2 topic echo --once --qos-reliability best_effort /cam_front/image_raw --field header 2>&1)
raw "${HDR_OUT}"
if echo "${HDR_OUT}" | grep -q 'frame_id: top_visual_link'; then
    pass "frame_id == top_visual_link"
else
    fail "frame_id is wrong or echo did not return"
fi
...
what     "Count message separators ('---') from 'topic echo --no-arr' over a 5-second window."
why      "Pipeline is configured for ~15 fps (delivered ~16 Hz). Accept >= 6 Hz (~40% of delivered) as pass. Below that there's something wrong upstream (Argus dropping or ISP backpressure)."
COUNT=$(count_msgs /cam_front/image_raw 5)
HZ=$(awk "BEGIN {printf \"%.1f\", $COUNT/5}")
raw "messages: ${COUNT}    over: 5 s    rate: ${HZ} Hz"
if [[ "${COUNT}" -ge 30 ]]; then
    pass "rate ${HZ} Hz (${COUNT} msgs / 5 s): healthy"
    ...
    fail "rate only ${HZ} Hz: below 6 Hz threshold"
    note "check journalctl for the per-pipeline log under share/gst_camera_manager/logs/cam_front/"
else
    fail "no image_raw messages received in 5 s"
fi
...
what     "Read the latched Bool with QoS RELIABLE / TRANSIENT_LOCAL / depth 1. Up to 10 s for first-time discovery."
why      "This is the manager's published verdict on whether frames are actually flowing. If status says RUNNING but alive says false, watchdog is seeing a stall."
ALIVE_OUT=$(_latched_bool /gst_camera_manager/cam_front/alive)
raw "${ALIVE_OUT}"
ALIVE_VAL=$(echo "${ALIVE_OUT}" | grep -oE 'data: (true|false)' | head -1)
case "${ALIVE_VAL}" in
    "data: true")  pass "alive == true (watchdog confirms frame flow)" ;;
...
# Distinguish "container not running" from "user lacks docker group membership" (which on
# a fresh setup the user can hit before re-logging post-reboot).
if ! docker ps >/dev/null 2>&1; then
    ...
    # Retry the supervisor lookup - the container may have just been started; ros2 launch inside
    # it can need 30+ s to advertise the SetBool service on a cold cache.
    SUP_SVCS=""
...
step "Both camera pipelines should be stopped, no leaked subprocesses"
what     "After stop, no gst_cam_node processes should remain."
why      "Process leaks indicate a bug in the subprocess-termination logic of the camera manager (signal not propagated to child, missing killpg)."
LEAK=$(pgrep -af 'gst_cam_node' 2>/dev/null | grep -v 'bash -c\|pgrep' || true)
if [[ -z "${LEAK}" ]]; then
