#!/bin/bash
# Calibrate the cam_down CSI pipeline with the ROS camera_calibration tool.
#
# Host-side flow: brings the camera up via the gst_camera_manager service, runs the
...
PIPE="cam_down"
TOPIC="/cam_down/image_raw"
NS="/cam_down"
LIVE="${GST_CALIB}/cam_down.yaml"
STORE_DIR="${STORE}/cam_down"
STORE_FILE="cam_down.yaml"
...
echo "Calibrating cam_down: pipeline '${PIPE}', topic ${TOPIC}, board ${SIZE} corners @ ${SQUARE} m"
...
if ! wait_for_frames 12; then
    ...
    wait_for_frames 15 || { err "no frames on ${TOPIC} - camera not streaming (check that the Isaac container is not holding sensor-id=0)"; exit 3; }
fi
ok "camera streaming"
...
echo "[calib] done. Restart the pipeline (cam_down_stop; cam_down_start) to pick up the new calibration."
