...
This directory holds two packages that publish the CSI cameras as ROS 2 camera streams.
...
|---|---|---|---|---|
| `cam_front` | IMX219, `sensor-id=0` | `/cam_front` | `top_visual_link` | 1920x1080 GRAY8 15 fps |
| `cam_down` | IMX219, `sensor-id=1` | `/cam_down` | `bottom_visual_link` | 1920x1080 GRAY8 15 fps |

## Operator commands

| Command | Invokes | Result |
|---|---|---|
| `cam_front_start` | `/gst_camera_manager/cam_front` `SetBool{true}` | forks `gst_cam_node`; `/cam_front` topics appear; `alive` true |
| `cam_front_stop` | `/gst_camera_manager/cam_front` `SetBool{false}` | subprocess terminated; topics stop; `alive` false |
| `cam_front_status` | `/gst_camera_manager/cam_front/status` `Trigger` | `cam_front RUNNING (pid=N)` or `cam_front STOPPED` |
| `cam_front_alive` | echoes `/gst_camera_manager/cam_front/alive` | latched `data: true` or `data: false` |
...
| `cam_stop` | `SetBool{false}` on both pipeline services | both subprocesses terminated |
...
| `ver_cv_cams [front\|down\|both]` | `SetBool{true}`, then subscribes `/<topic>/image_raw/compressed` | live window; pipelines it started are stopped on exit |
...
Calibration files live in `config/calibrations/` in `camera_calibration_parsers` format. Both shipped pipelines leave `calibration` empty, so the node logs `Invalid calibration path` and publishes placeholder intrinsics from the first frame: zero distortion, `fx = fy = width`, centre principal point.

`cam_calibrate front|down` writes `config/calibrations/<pipeline>.yaml` in the source tree. Set `calibration: "<pipeline>"` in `pipelines.yaml`, then install and reload.
