...
`arid_description` holds the ARID quadrotor URDF, its visual mesh and an RViz configuration. The link names it defines are the frame ids the camera and visual-odometry packages stamp and look up.
...
| `left_realsense_link` | -0.0175, 0.08875, 0.092042 | 0, 0, 90 | Left RealSense | `realsense2_camera` base frame |
| `right_realsense_link` | 0.0175, -0.08875, 0.092042 | 0, 0, -90 | Right RealSense | `realsense2_camera` base frame |
| `top_visual_link` | 0.077, 0, 0.129792 | -90, 0, -90 | Forward CSI camera | `frame_id` for `cam_front` in `pipelines.yaml` |
...
Each RealSense link is the base frame `realsense2_camera` builds as `<camera_name>_link` from `vslam_config.yaml`, and the driver's stream frames hang beneath it. cuVSLAM reads its camera extrinsics from the `base_link` to optical-frame chain in `/tf_static`.
