...
Launch package for a RealSense-based visual SLAM stack. Compatible with Intel RealSense **D43X** series stereo depth cameras. Wraps multiple RealSense drivers + [Isaac ROS Visual SLAM](https://github.com/NVIDIA-ISAAC-ROS/isaac_ros_visual_slam) + a PX4 bridge into a single `ros2 launch`.
...
Normal operation goes through `/arid_supervisor/vslam_enable` (camera-proven 3-RealSense gate with a landed interlock, see the [`arid_supervisor`](../arid_supervisor/README.md) README). Direct `ros2 launch` is the unmanaged dev path:
...
1. **Waits for `/robot_description`**. The drone's `robot_state_publisher` must be running with its links configured correctly. The launch blocks here with `[vslam] Waiting for /robot_description ...` until the latched message arrives.
2. **Starts a composable-node container** (`vslam_container`) with:
   ...
   - `nvidia::isaac_ros::visual_slam::VisualSlamNode`: 6-stream stereo-multicam SLAM (3 RealSense units, one IR stereo pair each).
3. **Starts `vslam_reactor_node`**: external-odom supervisor (see the reactor's README).
4. **Starts `vio_transform`**: VSLAM to PX4 bridge.
...
Single YAML keyed by node name. Three RealSense blocks (per-camera) and one SLAM block. `front_realsense` and `right_realsense` mirror the `left_realsense` block, with colour enabled on `front_realsense` only. `base_frame` and `imu_frame` must exist in the published TF tree.
...
| `enable_imu_fusion` | `visual_slam_node` | IMU-aided VSLAM. Currently unsupported: the `vio_transform` IMU republisher is commented out pending re-test. |
...
- **IMU fusion is off by default** (`enable_imu_fusion: false`) so the onboard RealSense IMU doesn't bias the PX4 state estimator via the visual-odom feedback path.

---

## `vio_transform` node

Converts the filtered VSLAM solution into the PX4 visual-odometry message. Subscribes the latched `/reactor/vio_reset_epoch` (`std_msgs/UInt8`) and forwards it into `VehicleOdometry.reset_counter` so EKF2 re-anchors its EV-aided states on a committed re-seat instead of gating the discontinuity.

| Subscribed | Type |
|---|---|
| `/visual_slam/filt_slam_odometry` | [`nav_msgs/Odometry`](https://github.com/ros2/common_interfaces/blob/humble/nav_msgs/msg/Odometry.msg) |
...
| `/reactor/vio_reset_epoch` | [`std_msgs/UInt8`](https://docs.ros2.org/humble/api/std_msgs/msg/UInt8.html) (transient-local) |
...
- `px4_msgs`, `tf2`, `tf2_ros`, `nav_msgs`, `sensor_msgs`, `geometry_msgs`.
