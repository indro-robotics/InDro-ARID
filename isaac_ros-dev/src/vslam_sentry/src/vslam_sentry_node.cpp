...
class VslamSentry : public rclcpp::Node {
 ...
 private:
  struct Cam {
    std::string ns;        // e.g. left_realsense
    std::string serial;
    ...
    for (auto &kv : cams_) {
      ...
      if (auto_reset_ && active_reset_.empty() && settled &&
        ...
        if (c.attempts >= max_attempts_) {
          ...
          // Calm-window deferral. Overrides that fire immediately regardless:
          // a SECOND unhealthy camera (below min_num_images, VO down - recovery
          // is urgent) or the bounded defer ceiling. min_num_images 4 makes a
          // single held-out camera safe: VO continues on the remaining pairs.
