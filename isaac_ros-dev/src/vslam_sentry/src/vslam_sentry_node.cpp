...
class VslamSentry : public rclcpp::Node {
 ...
 private:
  ...
  void tick() {
    ...
    // Process-plane discriminator: independent cameras do not die in the same instant
    // from hardware, but one frozen/stalled driver PROCESS kills every stream and VO
    // together. A driver hw_reset cannot help either that or a bus-wide power death
    // (device off-bus), so resets are HELD while the pattern stands; any single stream
    // recovering clears it. Suppression only - this can never fire a reset.
    // INERT BELOW TWO CAMERAS: with one camera "all streams dead" IS an ordinary single-
    // camera hardware death and VO down is its guaranteed consequence, not independent
    // evidence - the discriminator would invert and suppress the only recovery there is.
    if (cams_.size() >= 2) {
      // CURRENT-tick window rates, not last-tick states: the streams die in the SAME
