...
1. **Filters the SLAM odometry stream.** Applies velocity / position-delta gates to reject samples that look like teleports or tracking glitches. Clean stream republished on `/visual_slam/filt_slam_odometry`.
2. **Detects reset-misalignment.** After a `SetSlamPose` call, the reactor verifies the VSLAM backend's new pose agrees with the PX4 estimator within a configurable rotation + translation tolerance. If not, it retries.
...
| `/visual_slam/status` | `isaac_ros_visual_slam_interfaces/VisualSlamStatus` | Isaac VSLAM | Tracking quality / state |
...
| `/reactor/drone_odom` | `nav_msgs/Odometry` | PX4 odom converted to ROS conventions (FRD→FLU), cached for reset-alignment checks |
| `/reactor/drone_pose` | `geometry_msgs/PoseStamped` | Same, pose-only (RViz / Foxglove) |
...
| `sync_cache_sz` | count | `message_filters` cache depth for PX4 ↔ VSLAM time alignment |
| `quat_delta_theta` | rad | Reset-alignment rotation tolerance (default ~5°) |
...
**If the YAML is missing or malformed:** the node warns/errors and falls back to hardcoded defaults (matching the original values), so it still starts.
...
- [`isaac_ros_visual_slam`](https://github.com/NVIDIA-ISAAC-ROS/isaac_ros_visual_slam) + `isaac_ros_visual_slam_interfaces` — the SLAM backend exposing `SetSlamPose`, and the msg/srv definitions used by the reactor's subscribers and service client.
- [`px4_vslam`](../px4_vslam/) — sibling launch package; provides the VSLAM graph this reactor supervises.
- `px4_msgs` — PX4 telemetry/command messages via uXRCE-DDS.
- `std_msgs`, `std_srvs`, `nav_msgs`, `geometry_msgs`, `sensor_msgs` — standard messages/services.
- `tf2_ros` — TF broadcaster/listener.
- `message_filters` — time-synchronized subscribers + message caches.
- `ament_index_python` — resolves the package's `share/` path at runtime to locate `reactor_conf.yaml`.

**Python packages:**
- `python3-numpy`, `python3-scipy`, `python3-yaml` — math + YAML config loading.
