...
class vslam_reactor(Node):
    def __init__(self): 
        ...
        # response, so a dispatch-anchored window would be eaten by service latency and could
        # expire before the first post-re-seat frame is ever admitted. None = no window open
        # (never re-seated, or the last window CLOSED - reseat_bypass_gate clears it so close is a
        # one-shot event).
        ...
        # Origin re-injects are settle-timeout bounded and NOT counted - only the unlimited jump
        # path is evidence that cuVSLAM is unrecoverable.
        ...
        ### SERVICES ###############################################################################
        # ALL re-seat producers AND the SetSlamPose response live in vslam_cbg (MutuallyExclusive),
        ...
        # the server in gen_processing_cbg, a triggered origin seat and a jump re-seat could BOTH
        # pass vslam_check() on a multi-thread executor and dispatch two concurrent SetSlamPose
        # calls, the loser silently undoing the winner.
    ...
    def _load_params(self):
        ...
        defaults = [
            ('vslam_stabilization_time', 1.0),   # in-node default matches yaml: it is the bypass-window floor, so a yaml load failure must not halve it
            ('lin_vel_gate', 15.0),
    ...
    def yaw_delta(self, q1_orientation: Quaternion, q2_orientation: Quaternion) -> float:
        ...
        # is scipy's EXTRINSIC (fixed-axis) sequence, whose element 0 is a body-side angle that
        # folds tilt into the yaw difference (up to 0.69 deg at 20 deg tilt, against the 2.0 deg
        # settle budget).
    ...
    def vslam_check(self):
        ...
        # RETIRES the seq token itself, so a late straggler response is dropped unconditionally -
        # it can neither clear a successor's vslam_busy nor bump the epoch at an arbitrary later
        # time for a pose whose commit was never observed. It also opens the bypass window: in the
        # case this watchdog exists for (cuVSLAM APPLIED the pose, only the response was lost) the
        # pose HAS moved, and judging the next frame against the frozen pre-re-seat baseline would
        # read the step as a jump and restart the loop with no bypass at all. If the pose was NOT
        # applied the rebase is a no-op. The epoch stays confirmation-only and is not bumped here.
        if self.vslam_busy:
    ...
    def odom_velocity_gate(self, vslam_odom_msg: Odometry) -> bool:
        ...
        # the cadence gate is engaged: it needs delta_time > VO_rate_lim, and any stamp gap >=
        # cadence_gate_hard_s has already engaged the gate on this very frame, so the term could
        # only ever fire inside a gated window - where a vehicle moving a perfectly normal 0.5 m/s
    ...
    def reseat_bypass_gate(self, vslam_odom_msg: Odometry):
        ...
        re-seat creates. A re-seat is purposeful by definition, so its step must never be re-judged
        as a spurious cuVSLAM jump. Without this the stale baseline makes the first post-re-seat
        frame a jump, which re-seats, which leaves the baseline stale again - a self-sustaining loop.
        ...
        into the baseline and permanently invisible to the gate afterwards. Bounded by the window
        (nominally ~vslam_stabilization_time, hard-capped at RESEAT_BYPASS_CEILING_S), which only
        opens at a committed re-seat, the moment cuVSLAM is freshest against EKF2.
        """
        ...
        # repeat for the rest of the flight (the baseline self-heals on the first post-close
        # frame via the not-jump refresh or a fresh re-seat).
    ...
    def _reseat_burst_eval(self) -> bool:
        # Prune the rolling window and keep /reactor/vo_healthy in sync. Returns True while the
        # budget is exhausted (jump re-seats blocked). Never touches vslam_busy or the reset epoch -
        # blocking a re-seat is not a re-seat.
        now_s = self.get_clock().now().nanoseconds * 1e-9
        while self._reseat_burst and (now_s - self._reseat_burst[0]) > self.reseat_burst_window_s:
            self._reseat_burst.popleft()
        # < not <=: the yaml budget means what it says - reseat_burst_max COMMITTED re-seats are
        # allowed in the window, the next attempt is blocked.
        healthy = len(self._reseat_burst) < self.reseat_burst_max
        if healthy == self._burst_unhealthy:
            self._burst_unhealthy = not healthy
            self._vo_health_update()
            # Distinct single-severity call sites (rclpy pins severity per call site).
            if not healthy:
                self.get_logger().error(
    ...
    def _reseat_on_jump(self):
        ...
        # multi-Hz cannot recover cuVSLAM, it only feeds EKF2 a reset storm. The reactor raises the
        # flag and stops; it never commands a flight action.
    ...
    def set_slam_pose(self, init=False):
        ...
        # throws away the heading datum with no rebase anywhere downstream.
        ...
        if self.vslam_check():
            if self.ev_fusion_started is False:
                if self.init_flag:
                    ...
                    # Keep the jump-gate baseline live on the pre-fusion path: without this the
                    # baseline stays frozen at a pre-fusion pose and the FIRST fused frame is
                    # judged against it - a spurious jump re-seat (plus EKF2 reset) at the worst
                    # moment, immediately at fusion start.
                    for field in Odometry.__slots__:
                        setattr(self.last_vslam_odom_msg, field, getattr(vslam_odom_msg, field))
                ...
                if bypass or not self.odom_velocity_gate(vslam_odom_msg):
                    if bypass and not bypass_fresh:
                        ...
                        if settle_dt < self.set_origin_settle_time:
                            ...
                            # set_slam_pose(True): that is an ORIGIN injection - position zeroed to
                            # (0,0,0) AND yaw zeroed - and in flight it discards the heading datum
                            # with no rebase anywhere downstream. Abandon alignment: the epoch is
                            # already committed, EKF2 re-anchors on its own; keep streaming.
                            self.get_logger().error(
...
def main(args=None):
    ...
    # groups starve the third, and the starved one is response-side, extending every busy window.
